...
function load_instance(file_path::String)
    ...
    events = []
    for (i,l) in enumerate(eachline(file_path * "/events.txt"))#enumerate to give an index i to each line l
        if i != 1
            l = replace(l, "\'" => "\"") # Example : 'arrival' -> "arrival" 
            T = eval(Meta.parse(l))
            push!(events, (T[2], T[3])) # T[2]=arrival or departure , T[3]=Number of slice
        end
    end
    for f in readdir(file_path)
        if SubString(f, 1, 5) == "slice"
            n = split(f," ")[end] #Number of slice in string
...
function load_sn(filename::String) #sn= Substrate network= Physical network
...
function load_vnr(filename::String) #vnr = virtual network request = slice
    json_graph = JSON.parsefile(filename)
    g = MetaGraph(json_graph["n"]);

    for i in 1:json_graph["n"]
...
#return list of physical nodes that can host the current virtual node
...
#remove first occurence of item in list a
...
function copy_graph(gr::MetaGraph{Int64, Float64})
    g = MetaGraph(nv(gr));
     for i = 1:nv(gr) #nv : number of vertices | ne : number of edges
        p = props(gr, i)
