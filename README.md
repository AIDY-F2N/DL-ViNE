...
* NEPA  
>In review for Computer Networks
```
julia nepa.jl <scenario folder> <log file> <level> <N> <use distance heuristic> <level refine> <number of refinements> <random seed>
```
* NRPA
...
usage:
```
julia nrpa.jl <scenario folder> <log file> <level> <N> <use distance heuristic> <random seed>
```
* MCTS from 
...
* SP-MCTS 
>Zheng, G., Wang, C., Shao, W. et al. A single-player Monte Carlo tree search method combined with node importance for virtual network embedding. Ann. Telecommun. 76, 297–312 (2021). 

usage:
```
julia sp_mcts.jl <scenario folder> <log file> <total budget> <random seed>
```
